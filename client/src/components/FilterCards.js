...
const Cards = ({ image, text, isSelected, onClick }) => {
  console.log("hello", IsMobileView());
  return (
    <>
      {!IsMobileView() ? <div
      ...
      <div
        className={`${filterCards.cardDivMobile} ${isSelected ? filterCards.selectedMobile : ''}`}
        onClick={onClick}
      >
        <div className={filterCards.container}>
          <img src={image} alt={text} className={filterCards.imageMobile} />
          <div className={filterCards.textMobile}>{text}</div>
        </div>
      </div>}
    </>
  );
...
const FilterCards = () => {
  ...
  return (
    <>
    <div className={!IsMobileView() ? filterCards.main : filterCards.mainMobile}>
